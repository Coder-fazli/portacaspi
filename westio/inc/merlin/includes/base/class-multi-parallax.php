...
class Westio_Elementor_Multi_Parallax {
    public function __construct() {
        add_action('elementor/element/section/section_layout/after_section_end', [$this, 'register_controls'], 10, 2);
        add_action('elementor/frontend/section/after_render', [$this, 'paralax_render'], 10, 1);

        add_action('elementor/element/container/section_layout/after_section_end', [$this, 'register_controls'], 10, 2);
        add_action('elementor/frontend/container/after_render', [$this, 'paralax_render'], 10, 2);
    }

    public function paralax_render($element) {
        $svgfile = $this->render_svg();
        wp_enqueue_script('tweenmax');
        wp_enqueue_script('multi-parallax');
        $settings      = $element->get_settings();
        $repeater_list = (isset($settings['westio_section_parallax_layer']) && $settings['westio_section_parallax_layer']) ? $settings['westio_section_parallax_layer'] : array();
        if ($settings['westio_section_parallax_switcher'] === 'yes') {
            foreach ($repeater_list as $layer):

                $html = '';
                if ($layer['westio_section_parallax_item_style'] == 'image') {
                    $image_url = $layer['westio_section_parallax_image']['url'];
                    if (!empty($image_url)) {
                        $path_parts = pathinfo($image_url);
                        if ($path_parts['extension'] === 'svg') {
                            $image = $layer['westio_section_parallax_image'];
                            if ($image['id']) {
                                $pathSvg = get_attached_file($image['id']);
                                $html    = westio_get_icon_svg($pathSvg);
                            }
                        } else {
                            $html = '<img src="'.$layer['westio_section_parallax_image']['url'].'" alt="'.$layer['westio_section_parallax_image']['alt'].'">';
                        }
                    }

                } else {
                    $html = $svgfile[$layer['westio_section_parallax_item_svg_style']];
                }
                if ($html) {
                    $parallax_mouse = $layer['westio_section_parallax_mouse'] == 'yes' ? 'true' : 'false';
                    echo '<div class="parallax-layer elementor-repeater-item-' . $layer['_id'] . ' elementor-repeater-element-' . $element->get_id() . ' f-' . $layer['westio_section_parallax_item_svg_color_type'] . '" data-layer = "' . $layer['_id'] . '" data-element = "' . $element->get_id() . '" data-parallax="' . $parallax_mouse . '" data-rate="' . $layer['westio_section_parallax_rate'] . '">' . $html . '</div>';
    ...
    public function register_controls($element, $args) {
        ...
        $element->start_controls_section(
            'westio_section_parallax',
            [
                'label' => __('Multi Parallax ', 'westio'),
                'tab'   => Controls_Manager::TAB_LAYOUT,
            ]
        );
        $element->add_control('westio_section_parallax_switcher',
            [
                'label' => __('Enable Parallax', 'westio'),
        ...
        $repeater->add_control(
            'westio_section_parallax_item_style',
            [
                'label'   => __('Parallax Type', 'westio'),
                'type'    => Controls_Manager::SELECT,
                'options' => [
                    'image' => __('Image', 'westio'),
                    'svg'   => __('Svg', 'westio'),
                    'svg'   => __('Opal Style', 'westio'),
                ],
                'default' => 'image',
            ]
        );
        ...
        $repeater->add_control(
            'westio_section_parallax_item_svg_style',
            [
                'label'     => __('Opal Style', 'westio'),
                'type'      => Controls_Manager::SELECT,
                'options'   => [
                    '0' => __('Style 1', 'westio'),
                    '1' => __('Style 2', 'westio'),
                ...
                'default'   => '1',
                'condition' => [
                    'westio_section_parallax_item_style' => 'svg'
                ]
            ]
        );

        $repeater->add_control(
            'westio_section_parallax_image',
            [
                ...
                'condition' => [
                    'westio_section_parallax_item_style' => 'image'
                ]
            ]
        );

        $repeater->add_control(
            'westio_section_parallax_item_svg_color_type',
            [
                'label'   => __('Opal Style Color', 'westio'),
                'type'    => Controls_Manager::SELECT,
                'options' => [
                    'primary'   => __('Primary', 'westio'),
                    'secondary' => __('Secondary', 'westio'),
                    'custom'    => __('Custom', 'westio'),
                ],
                'default' => 'custom',
            ]
        );
        ...
        $repeater->add_control(
            'westio_section_parallax_item_svg_color',
            [
                'label'     => __('Custom Color', 'westio'),
                'type'      => Controls_Manager::COLOR,
                'selectors' => [
                    '{{WRAPPER}} {{CURRENT_ITEM}}.parallax-layer svg' => 'fill: {{VALUE}};',
                ],
                'condition' => [
                    'westio_section_parallax_item_svg_color_type' => 'custom',
                ],
            ]
        );

        $repeater->add_responsive_control(
            'westio_section_parallax_item_svg_width',

            [
                'name'       => 'westio_section_parallax_item_svg_width',
        ...
        $repeater->add_responsive_control(
            'westio_section_parallax_item_svg_height',
            [
                'label'      => __('Height', 'westio'),
                'type'       => Controls_Manager::SLIDER,
                'default'    => [
                    'size' => 100,
                    'unit' => '%',
        ...
        $repeater->add_responsive_control(
            'westio_section_parallax_item_svg_rotate',
            [
                'label'     => __('Rotate', 'westio'),
                'type'      => Controls_Manager::SLIDER,
                'default'   => [
                    'size' => 0,
                    'unit' => 'deg'
        ...
        $repeater->add_responsive_control(
            'westio_section_parallax_position_h',
            [
                'label'      => __('Position h', 'westio'),
                'type'       => Controls_Manager::SLIDER,
                'range'      => [
                    '%'  => [
                        'min' => 0,
        ...
        $repeater->add_responsive_control(
            'westio_section_parallax_position_v',
            [
                'label'      => __('Position v', 'westio'),
                'type'       => Controls_Manager::SLIDER,
                'range'      => [
                    '%'  => [
                        'min' => 0,
        ...
        $repeater->add_responsive_control(
            'westio_section_parallax_z_index',
            [
                'label'       => __('Z-Index', 'westio'),
                'type'        => Controls_Manager::NUMBER,
                'default'     => 1,
                'description' => __('Choose z-index for the parallax layer, default: 3', 'westio'),
            ]
        );
        $repeater->add_control(
            'westio_section_parallax_mouse',
            [
                'label'       => __('Mousemove Parallax', 'westio'),
                'type'        => Controls_Manager::SWITCHER,
                'description' => __('Enable or disable mousemove interaction', 'westio'),
            ]
        );
        $repeater->add_responsive_control(
            'westio_section_parallax_rate',
            [
                'label'   => __('Rate', 'westio'),
                'type'    => Controls_Manager::NUMBER,
                'default' => -10,
                'min'     => -20,
                'max'     => 20,
        ...
        $element->add_control(
            'westio_section_parallax_layer',
            [
                'label'     => __('Parallax Items', 'westio'),
                'type'      => Controls_Manager::REPEATER,
                'condition' => [
                    'westio_section_parallax_switcher' => 'yes',
                ],
...
new Westio_Elementor_Multi_Parallax();