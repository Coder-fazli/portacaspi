...
namespace Westio\elementor\Controls;
...
class Westio_Group_Control_Query extends Group_Control_Base {
    ...
    public static function get_public_post_types( $args = [] ) {
         ...
         *
         */
        $post_types = apply_filters( 'westio/elementor/get_public_post_types', $post_types );
        return $post_types;
    }
    ...
    protected function get_fields_array( $name ) {
        ...
        $fields['post_type'] = [
            'label'   => esc_html__( 'Source', 'westio' ),
            'type'    => Controls_Manager::SELECT,
            'options' => [
                'by_id'         => esc_html__( 'Manual Selection 1      ss ', 'westio' ),
        ...
        $fields['query_include'] = [
            ...
            'label'        => esc_html__( 'Include', 'westio' ),
        ...
        $fields['posts_ids'] = [
            'label'        => esc_html__( 'Search & Select 3', 'westio' ),
            'type'         => 'query',
            'options'      => [],
            'label_block'  => true,
            'multiple'     => true,
        ...
        $fields['include'] = [
            'label'        => esc_html__( 'Include By', 'westio' ),
            'type'         => Controls_Manager::SELECT2,
            'multiple'     => true,
            'options'      => [
                'terms'   => esc_html__( 'Term', 'westio' ),
                'authors' => esc_html__( 'Author', 'westio' ),
            ],
            'condition'    => [
                'post_type!' => [
                    'by_id',
        ...
        $fields['include_term_ids'] = [
            'label'        => esc_html__( 'Term', 'westio' ),
            'description'  => esc_html__( 'Terms are items in a taxonomy. The available taxonomies are: Categories, Tags, Formats and custom taxonomies.', 'westio' ),
            'type'         => 'query',
            'options'      => [],
            'label_block'  => true,
            'multiple'     => true,
        ...
        $fields['include_authors'] = [
            'label'        => esc_html__( 'Author', 'westio' ),
            'label_block'  => true,
            'type'         => 'query',
            'multiple'     => true,
            'default'      => [],
        ...
        $fields['query_exclude'] = [
            'type'         => Controls_Manager::TAB,
            'label'        => esc_html__( 'Exclude', 'westio' ),
            'tabs_wrapper' => $tabs_wrapper,
            'condition'    => [
                'post_type!' => [
                    'by_id',
        ...
        $fields['exclude'] = [
            'label'        => esc_html__( 'Exclude By', 'westio' ),
            'type'         => Controls_Manager::SELECT2,
            'multiple'     => true,
            'options'      => [
        ...
        $fields['exclude_ids'] = [
            'label'        => esc_html__( 'Search & Select 1', 'westio' ),
            'type'         => 'query',
            'options'      => [],
            'label_block'  => true,
            'multiple'     => true,
        ...
        $fields['avoid_duplicates'] = [
            'label'        => esc_html__( 'Avoid Duplicates', 'westio' ),
            'type'         => Controls_Manager::SWITCHER,
            'default'      => '',
            'description'  => esc_html__( 'Set to Yes to avoid duplicate posts from showing up. This only effects the frontend.', 'westio' ),
            'tabs_wrapper' => $tabs_wrapper,
            'inner_tab'    => $exclude_wrapper,
            'condition'    => [
                'post_type!' => [
        ...
        $fields['offset'] = [
            'label'        => esc_html__( 'Offset', 'westio' ),
            ...
            'description'  => esc_html__( 'Use this setting to skip over posts (e.g. \'2\' to skip over 2 posts).', 'westio' ),
            'tabs_wrapper' => $tabs_wrapper,
            'inner_tab'    => $exclude_wrapper,
        ];

        $fields['select_date'] = [
            'label'     => esc_html__( 'Date', 'westio' ),
            'type'      => Controls_Manager::SELECT,
            'post_type' => '',
            'options'   => [
                'anytime' => esc_html__( 'All', 'westio' ),
                'today'   => esc_html__( 'Past Day', 'westio' ),
                'week'    => esc_html__( 'Past Week', 'westio' ),
                'month'   => esc_html__( 'Past Month', 'westio' ),
                'quarter' => esc_html__( 'Past Quarter', 'westio' ),
                'year'    => esc_html__( 'Past Year', 'westio' ),
                'exact'   => esc_html__( 'Custom', 'westio' ),
            ],
            'default'   => 'anytime',
            'multiple'  => false,
            'condition' => [
        ...
        $fields['date_before'] = [
            'label'       => esc_html__( 'Before', 'westio' ),
            'type'        => Controls_Manager::DATE_TIME,
            'post_type'   => '',
            'label_block' => false,
            'multiple'    => false,
            'placeholder' => esc_html__( 'Choose', 'westio' ),
            'condition'   => [
                'select_date' => 'exact',
                'post_type!'  => [
                    'by_id',
            ...
            'description' => esc_html__( 'Setting a ‘Before’ date will show all the posts published until the chosen date (inclusive).', 'westio' ),
        ];

        $fields['date_after'] = [
            'label'       => esc_html__( 'After', 'westio' ),
            'type'        => Controls_Manager::DATE_TIME,
            'post_type'   => '',
            'label_block' => false,
            'multiple'    => false,
            ...
            'description' => esc_html__( 'Setting an ‘After’ date will show all the posts published since the chosen date (inclusive).', 'westio' ),
        ];

        $fields['orderby'] = [
            'label'     => esc_html__( 'Order By', 'westio' ),
            'type'      => Controls_Manager::SELECT,
            'default'   => 'post_date',
            'options'   => [
                'post_date'     => esc_html__( 'Date', 'westio' ),
                'post_title'    => esc_html__( 'Title', 'westio' ),
                'menu_order'    => esc_html__( 'Menu Order', 'westio' ),
                'modified'      => esc_html__( 'Last Modified', 'westio' ),
                'comment_count' => esc_html__( 'Comment Count', 'westio' ),
        ...
        $fields['order'] = [
            'label'     => esc_html__( 'Order', 'westio' ),
            'type'      => Controls_Manager::SELECT,
            'default'   => 'desc',
            'options'   => [
                'asc'  => esc_html__( 'ASC', 'westio' ),
                'desc' => esc_html__( 'DESC', 'westio' ),
            ],
            'condition' => [
                'post_type!' => 'current_query',
            ],
        ...
        $fields['posts_per_page'] = [
            'label'     => esc_html__( 'Posts Per Page', 'westio' ),
            'type'      => Controls_Manager::NUMBER,
            'default'   => 3,
            'condition' => [
                'post_type!' => 'current_query',
        ...
        $fields['ignore_sticky_posts'] = [
            'label'       => esc_html__( 'Ignore Sticky Posts', 'westio' ),
            'type'        => Controls_Manager::SWITCHER,
            'default'     => 'yes',
            'condition'   => [
                'post_type' => 'post',
            ],
            'description' => esc_html__( 'Sticky-posts ordering is visible on frontend only', 'westio' ),
        ];

        $fields['query_id'] = [
            'label'       => esc_html__( 'Query ID', 'westio' ),
            'type'        => Controls_Manager::TEXT,
            'default'     => '',
            'ai'          => [
                'active' => false,
            ],
            'description' => esc_html__( 'Give your Query a custom unique id to allow server side filtering', 'westio' ),
            'separator'   => 'before',
            'dynamic'     => [
                'active' => true,
            ],
