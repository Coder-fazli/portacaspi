 ...
 * @since 1.0.0
 */
class Westio_Elementor_Image_Gallery extends Westio_Base_Widgets {
    ...
    public function get_title() {
        return esc_html__('Westio Image Gallery', 'westio');
    }
    ...
    public function get_script_depends() {
        return [
            'isotope',
            'masonry-pkgd',
            'westio-elementor-image-gallery'
        ];
    }
    ...
    protected function register_controls() {
        $this->start_controls_section(
            'section_gallery',
            [
                'label' => esc_html__('Image Gallery', 'westio'),
            ]
        );
        ...
        $repeater->add_control(
            'filter_title',
            [
                'label'       => esc_html__('Filter Title', 'westio'),
                'type'        => Controls_Manager::TEXT,
                'label_block' => true,
                'placeholder' => esc_html__('List Item', 'westio'),
                'default'     => esc_html__('List Item', 'westio'),
            ]
        );
        ...
        $repeater->add_control(
            'wp_gallery',
            [
                'label'      => esc_html__('Add Images', 'westio'),
        ...
        $this->add_control(
            ...
            [
                ...
                'default'     => [
                    [
                        'filter_title' => esc_html__('Gallery 1', 'westio'),
                    ],
                ],
        ...
        $this->add_control(
            'show_filter_bar',
            [
                'label'     => esc_html__('Filter Bar', 'westio'),
                'type'      => Controls_Manager::SWITCHER,
                'label_off' => esc_html__('Off', 'westio'),
                'label_on'  => esc_html__('On', 'westio'),
            ]
        );
        ...
        $this->add_control(
            'show_filter_all',
            [
                'label'     => esc_html__('Filter All', 'westio'),
                'type'      => Controls_Manager::SWITCHER,
                'label_off' => esc_html__('Off', 'westio'),
                'label_on'  => esc_html__('On', 'westio'),
                'condition' => [
...
'(mobile){{WRAPPER}} .grid__item'       => 'width: calc((100% - {{column_spacing.SIZE}}{{column_spacing.UNIT}}*({{column_mobile.VALUE}} - 1)) / {{column_mobile.VALUE}});',
        ...
        $this->add_responsive_control(
            'filter_padding',
            [
                'label'      => esc_html__('Filter Padding', 'westio'),
                ...
                'selectors'  => [
                    '{{WRAPPER}} .elementor-galerry__filter' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
                ],
            ]
        );
    ...
    protected function render() {
        ...
        if ($settings['show_filter_bar'] == 'yes') :
            ...
            ?>
            <div class="elementor-galerry__filters-wrapper">
                <ul class="elementor-galerry__filters" data-related="isotope-<?php echo esc_attr($this->get_id()); ?>">

                    <?php if ($settings['show_filter_all'] == 'yes') : ?>
                        <li class="elementor-galerry__filter elementor-active" data-filter=".gallery_group_all">
                            <?php echo esc_html__('All', 'westio'); ?>
                    ...
                    $i = 0;
                    foreach ($settings['filter'] as $key => $term) :
                        $active_class = ($settings['show_filter_all'] != 'yes' && $i === 0) ? ' elementor-active' : '';
                        ?>
                        <li class="elementor-galerry__filter<?php echo esc_attr($active_class); ?>"
                            data-filter=".gallery_group_<?php echo esc_attr($key); ?>">
                            <?php echo esc_html($term['filter_title']); ?>
        ...
        <?php
        $show_filter_all = isset($settings['show_filter_all']) && $settings['show_filter_all'] === 'yes';
        $first_group = '';
        if (!empty($image_gallery)) {
            $first_group = $image_gallery[0]['group'];
        }
        ?>
            ...
            <div <?php $this->print_render_attribute_string('inner'); ?>>
                <?php foreach ($image_gallery as $index => $item) :
                    ...
                    if (Elementor\Plugin::$instance->editor->is_edit_mode()) {
                        $this->add_render_attribute($link_content_setting_key, [ 'class' => 'elementor-clickable', ]);

                    }

                    $display_style = (!$show_filter_all && $item['group'] !== $first_group) ? 'display:none; top: 0' : '';
                    ?>

                    <div class="grid__item <?php echo esc_attr(($show_filter_all ? 'gallery_group_all ' : '')); ?>gallery_group_<?php echo esc_attr($item['group']); ?>"
                         style="<?php echo esc_attr($display_style); ?>">
                        <div class="gallery-image">
                            <a <?php $this->print_render_attribute_string($link_content_setting_key); ?>
...
$widgets_manager->register(new Westio_Elementor_Image_Gallery());