...
class Westio_Sticky_Module {
    ...
    public function scripts() {
        wp_register_script('elementor-sticky', get_template_directory_uri() . '/assets/js/frontend/jquery-sticky.js', ['jquery'], WESTIO_VERSION, true);
        wp_enqueue_script('westio-elementor-sticky', get_template_directory_uri() . '/assets/js/libs/sticky.js', ['jquery', 'elementor-frontend', 'elementor-sticky'], WESTIO_VERSION, true);
    }
    ...
    public function register_controls(Element_Base $element) {
        $element->add_control(
            'sticky',
            [
                'label'              => esc_html__('Sticky', 'westio'),
                'type'               => Controls_Manager::SELECT,
                'options'            => [
                    ''       => esc_html__('None', 'westio'),
                    'top'    => esc_html__('Top', 'westio'),
                    'bottom' => esc_html__('Bottom', 'westio'),
                ],
                'separator'          => 'before',
                'render_type'        => 'none',
        ...
        foreach ($active_devices as $device) {
            $label                          = 'desktop' === $device ? esc_html__('Desktop', 'westio') : $active_breakpoint_instances[$device]->get_label();
        ...
        $element->add_control(
            'sticky_on',
            [
                'label'              => esc_html__('Sticky On', 'westio'),
        ...
        $element->add_responsive_control(
            ...
            [
                'label'              => esc_html__('Effects Offset', 'westio'),
                'type'               => Controls_Manager::NUMBER,
                'default'            => 0,
                'min'                => 0,
                'max'                => 1000,
        ...
        if ($this->is_instance_of($element, $types)) {
            ...
            $element->add_control(
                'sticky_parent',
                [
                    'label'              => esc_html__('Stay In Column', 'westio'),
    ...
    public function register_controls_search_popup(Element_Base $element) {
        ...
        $element->add_control(
            'search_popup_parent',
            [
                'label'        => esc_html__('Parent search popup', 'westio'),
                'type'         => Controls_Manager::SWITCHER,
