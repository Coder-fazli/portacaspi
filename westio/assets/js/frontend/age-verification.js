(function ($) {
    ...
    $.aroageCheck = function (options) {
        ...
        const _this = {
            ...
            errors: [],
            setValues() {
                const month = $(".westio-av .month").val();
                const day = $(".westio-av .day").val();
                _this.month = month;
                _this.day = day.replace(/^0+/, ""); // remove leading zero
                _this.year = $(".westio-av .year").val();
            },
            validate() {
                _this.errors = [];
            ...
            buildHtml() {
                ...
                html += "<div class=\"westio-av-overlay\"></div>";
                html += "<div class=\"westio-av\" style=\"" + style + " \">";
                ...
                if (settings.description !== "") {
                    html += `<p class="westio-av-description">${settings.description}</p>`;
                }
                html += `<p class="westio-av-copy">${copy.replace("[age]", `<strong>${settings.minAge}</strong>`)}`;
                +`</p>`;
                ...
                $(".westio-av-overlay").animate({
                    opacity: 1,
                }, 500, () => {
                    _this.reCenter($(".westio-av"));
                    $(".westio-av").css({
                        opacity: 1,
                    });
                });

                $(".westio-av .day, .westio-av .year").focus(function () {
                    ...
                    $(".westio-av").animate({
                        ...
                        $(".westio-av-overlay").animate({
                            ...
                            if (settings.redirectTo !== "") {
                                ...
                                $(".westio-av-overlay, .westio-av").remove();
                            }
                        });
                    });
                }, settings.messageTime);
